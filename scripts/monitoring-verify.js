...
try {
  ...
  check('计算重试率和平均输入规模', stats.retryCalls === 1 && stats.retryRate === 33.3 && stats.avgInputChars === 667);
  ...
  check('失败日志不保存上游原始错误文本', !raw.includes('SENSITIVE_RESUME_SHOULD_NOT_PERSIST') && raw.includes('上游超时'));
